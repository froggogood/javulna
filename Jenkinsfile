  ...
  stages { // Defines the different stages of the pipeline
    stage('Checkout Source') { // Defines the 'Checkout Source' stage
      steps { // Specifies the steps to be executed within this stage
        git 'https://github.com/froggogood/javulna.git' // Retrieves the source code from the specified GitHub repository
      }
    }

    stage('SonarQube Analysis') {
      steps {
        sh "mvn  verify -e org.sonarsource.scanner.maven:sonar-maven-plugin:4.0.0.4121:sonar \
